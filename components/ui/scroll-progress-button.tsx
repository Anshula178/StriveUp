...
export function ScrollProgressButton({ isOpen, onClick, className }: ScrollProgressButtonProps) {
  ...
  return (
    <button
      onClick={onClick}
      className={`relative group flex items-center justify-center w-14 h-14 rounded-full bg-background/80 backdrop-blur-md shadow-lg border border-border transition-all duration-300 hover:scale-105 active:scale-95 ${className}`}
    ...
    >
      ...
      {/* Icon */}
      <div className="relative z-20 text-foreground group-hover:text-primary transition-colors">
        <motion.div
          initial={false}
